...
local function choose_user(alias, config)
  ...
  elseif choice == 3 then
    ...
    if not custom_user or custom_user == "" then return nil end
...
local function choose_filtered(title, items, config)
  ...
  if query == "" then
    ...
    for _, item in ipairs(items) do
      if item:lower():find(query, 1, true) then table.insert(filtered_items, item) end
    end
  end
...
local function read_ssh_config_hosts()
  ...
  for line in f:lines() do
    ...
    if host and host ~= "*" then list[#list + 1] = host end
  end
  f:close()
  return list
end
...
local function is_mount_active(path, url, mount_dir)
  if not is_dir(url) then return false end
  ...
  for _, mount_info in ipairs(mounts) do
    if mount_info.path == path then return true end
  end
...
local function is_connection_error(error_msg)
  return error_msg
    and (
        error_msg:match("Connection refused")
        or error_msg:match("Connection timed out")
        or error_msg:match("Could not resolve hostname")
      )
      ~= nil
end
...
local function try_key_auth(alias, mountPoint, mount_to_root, custom_remote_path, config)
  ...
  if output and output.status and output.status.success then return nil, output end
...
local function try_password_auth(alias, mountPoint, mount_to_root, custom_remote_path, config)
  ...
  for attempt = 1, max_attempts do
    ...
    if not pw or pw == "" then return nil, "User aborted" end
...
local function check_existing_mount(hostname, mount_dir)
  ...
  for _, mount in ipairs(mounts) do
    ...
    if mount_hostname == hostname then return mount.path end
  end
  return nil
end
...
local function finalize_mount(alias, mountPoint, jump)
  ...
  if jump then ya.emit("cd", { mountPoint, raw = true }) end
end
...
local function add_mountpoint(entry, jump)
  ...
  if not err_key_auth then return finalize_mount(entry, mountPoint, jump) end
...
local function get_active_mounts_or_warn(warn_message)
  ...
  local mounts = list_mounts(mount_dir)
  if #mounts == 0 then
    if warn_message then Notify.warn(warn_message) end
    return nil
  end
  return mounts
end
...
local function cmd_remove_alias()
  ...
  for _, line in ipairs(saved_aliases) do
    if line ~= alias then table.insert(updated, line) end
  end
  ...
  if host_cache.hosts then
    ...
    local new_hosts = {}
    for _, existing_alias in ipairs(host_cache.hosts) do
      if existing_alias ~= alias then table.insert(new_hosts, existing_alias) end
    end
